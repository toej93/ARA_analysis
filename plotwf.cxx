...
{ //start main
  ...
  for(int arg=3; arg<argc; arg++){
    ...
    */    
    bool found=false;
    for(int event=0; event<10; event++){ //loop over those entries
      if(found) break;
	...
	for(int i=0; i<15; i++){//canvas loop
	  ...
	  dummy[i]->GetYaxis()->SetRangeUser(-8,8);
	  ...
	  padded_waveform_diode_gr[i]->Draw("SAME");    
	...
	c3->SaveAs("wforms/wf_diode_16.png");
	delete c3;
