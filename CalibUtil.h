...
bool IsGoodForCalib_sim(double thres){
...
double get3rdPeakSqValSamePol_timeordered(double *vsquared_time, double *vsquared) //get 3rd highest v^2 for the same polarization
{
  vector <double> peakValVpol;
  vector <double> peakValHpol;
  ...
  if(whichPol==1){
    for(int i = 8; i<15; i++){
      if(abs(time_vmax[i].second-peakValHpol[2])<1.E-04) third=i;
      if(abs(time_vmax[i].second-peakValHpol[3])<1.E-04) fourth=i;
    }
  }
...
bool isTriggered(double threshold, double *peaks, double &thirdHighest_pol){
  vector<double> thirdHighest;
  //thirdHighest.resize(2);
  get3rdsmallest(peaks, thirdHighest);
  // cout << thirdHighest[1]<< endl;
  if(abs(thirdHighest[0])>abs(thirdHighest[1])) thirdHighest_pol=thirdHighest[0];
  else if(abs(thirdHighest[1])>abs(thirdHighest[0])) thirdHighest_pol=thirdHighest[1];
  //cout << thirdHighest_pol<< endl;
