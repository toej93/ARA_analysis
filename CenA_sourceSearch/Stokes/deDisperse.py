...
for i in range(0,totalEvents):#loop over events
    ...
    try:
        ...
        continue
    # polVec = np.zeros(3)
    polVec_x.append(reportPtr.stations[0].strings[0].antennas[0].Pol_vector[0].GetX())
    polVec_y.append(reportPtr.stations[0].strings[0].antennas[0].Pol_vector[0].GetY())
    polVec_z.append(reportPtr.stations[0].strings[0].antennas[0].Pol_vector[0].GetZ())
    gr = [None]*8
    pyrex_array = []
    ...
    angle_stokes.append(util.PolAngleStokes(maxH,maxV))
