...
////////////////////////////////////////////////////////////////////////////////
////	optimize_CenA.cxx
////	Cen A source search. Do optimization on data.
////
////	Code created by B. Clark and adapted by J. Torres.
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	if(argc<6){
		cout<< "Usage\n" << argv[0] << " <1-station> <2-config> <3-year_or_energy> <4-slope> <5-pol_select> <6-intercept> <7-ValForCuts filename>"<<endl;;
		return -1;
	}
	...
	for(int loopNum = 0; loopNum<5; loopNum++){
		...
		if(version2){
			...
			*/
			...
			for(int file_num=7; file_num<argc; file_num++){
				...
				//Get random coordinates of a source
				// cout << intercept_in << endl;
				...
				for(int event=0; event<numEntries; event++){
					...
					corr_val[0] = corr_val_new[0];
					corr_val[1] = corr_val_new[1];
					snr_val[0] = snr_val_new[0];
					...
					if( (isSoft || isBadEvent || hasBadSpareChanIssue || isFirstFiveEvent || isShort || isCal || !inSource))
						continue;
					...
					for(int pol=0; pol<2; pol++){
						...
						if(!WFRMS[pol] && !isNewBox && !isSurf[0] && !isSurf[1] && !isSurfEvent_top[pol]){
							...
							if(!failsCWPowerCut){
								...
								// double this_y_val = (slope * corr_val[pol] ) + intercept[bin];
		...
		// 	}
		// 	char title[300];
		// 	sprintf(title, "%s/optimize/A%d_config%d_%dEvents_SNRvsCorr_%i.png",plotPath,station,config,numTotal,loopNum);
		...
		// 	// gPad->SetLogy();
		// 	ctoyBack->SaveAs("./plots/toy_background_distro.png");
		//
		// 	// return 0;
		//
		...
		// 	char the_sims[500];
		// 	if(station==2) sprintf(the_sims,"/fs/project/PAS0654/ARA_DATA/A23/sim/ValsForCuts_UsedInA2FinalOpt/A%d/c%d/E224/cutvals_drop_FiltSurface_snrbins_0_0_wfrmsvals_-1.3_-1.4_run_*.root",station,config);
		// 	if(station==3) sprintf(the_sims,"/fs/project/PAS0654/ARA_DATA/A23/sim/ValsForCuts/A%d/c%d/E224/cutvals_drop_snrbins_0_0_wfrmsvals_-1.3_-1.4_run_*.root",station,config);
		//
		...
		// 		"%s/optimize/A%d_config%d_E%2.1f_Pol%d_Efficiency_%i.png",plotPath,station,config,224.,pol_select,loopNum);
