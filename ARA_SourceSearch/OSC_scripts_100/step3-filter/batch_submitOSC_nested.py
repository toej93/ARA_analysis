...
with open(file, mode='r', newline='', encoding='utf-8-sig') as csvfile:
    ...
    for job in reader:
        if(count_core==0):
            f = open("/users/PAS0654/osu8354/ARA_cvmfs/source/AraRoot/analysis/ARA_analysis/ARA_SourceSearch/OSC_scripts/step3-filter/tmpScipts/A%i_%i_tmpSubmit_%i.sh"%(station, year, count), "w+")
            f.write("#!/bin/bash\n\n")
