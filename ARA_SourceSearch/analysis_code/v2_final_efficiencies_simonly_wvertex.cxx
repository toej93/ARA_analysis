...
int main(int argc, char **argv)
{
	...
	*/
	...
	for(int i=0; i<energiesToLoopOver.size(); i++){
		sprintf(the_sims,"/fs/project/PAS0654/ARA_DATA/A23/sim_v2/ValsForCuts_wVertex/A%d/c%d/E%d/*.root",station,config,energiesToLoopOver[i]);
		printf("/fs/project/PAS0654/ARA_DATA/A23/sim_v2/ValsForCuts_wVertex/A%d/c%d/E%d/*.root\n",station,config,energiesToLoopOver[i]);
	...
	TH1D *pass_sq_vsSNR[2];
	TH1D *thrown_sq_vsSNR[2];
	...
	TH1D *efferr_custom_vsSNR[2];

	for(int i=0; i<2; i++){
		all_events[i] = new TH1D("","",25,-0.5,24.5);
		pass_soft_short_cal_wfrms[i] = new TH1D("","",25,-0.5,24.5);
		pass_soft_short_cal_wfrms_box[i] = new TH1D("","",25,-0.5,24.5);
		pass_soft_short_cal_wfrms_box_surf[i] = new TH1D("","",25,-0.5,24.5);
		pass_soft_short_cal_wfrms_box_surf_rcut[i] = new TH1D("","",25,-0.5,24.5);
		pass_sq_vsSNR[i] = new TH1D("","",25,-0.5,24.5);
		thrown_sq_vsSNR[i] = new TH1D("","",25,-0.5,24.5);

		eff_soft_short_cal_wfrms[i] = new TH1D("","",25,-0.5,24.5);
		eff_soft_short_cal_wfrms_box[i] = new TH1D("","",25,-0.5,24.5);
		eff_soft_short_cal_wfrms_box_surf[i] = new TH1D("","",25,-0.5,24.5);
		eff_soft_short_cal_wfrms_box_surf_rcut[i] = new TH1D("","",25,-0.5,24.5);
		efferr_custom_vsSNR[i] = new TH1D("","",25,-0.5,24.5);
	...
	TH1D *pass_sq_vsE = new TH1D("","",11,15.75,21.25);
	TH1D *thrown_sq_vsE =new TH1D("","",11,15.75,21.25);
	...
	TH1D *efferr_custom_vsE = new TH1D("","",11,15.75,21.25);
...

	{
		...
		simAllTree.SetBranchAddress("isVertexable_V",&isVertexable[0]);
		simAllTree.SetBranchAddress("isVertexable_H",&isVertexable[1]);
		simAllTree.SetBranchAddress("surf_V_vertex",&isSurfEvent_vertex[0]);
		simAllTree.SetBranchAddress("surf_H_vertex",&isSurfEvent_vertex[1]);
		...
		for(int event=0; event<numSimEvents; event++){
			...
			for(int pol=0; pol<2; pol++){
				...
				thrown_sq_vsSNR[pol]->Fill(this_SNR,weight*weight);
				if(!WFRMS[pol] && !failsCWPowerCut[pol]){
					...
					if(!isNewBox && !box300){
						...
						if(!isSurf[0] && !isSurf[1] && !isSurfEvent_top[pol]){
							...
							if(!failsRcut[pol] && isVertexable[pol] && !isSurfEvent_vertex[pol]){
								pass_soft_short_cal_wfrms_box_surf_rcut[pol]->Fill(this_SNR,weight);
								pass_sq_vsSNR[pol]->Fill(this_SNR,weight*weight);
							}
			...
			for(int pol=0; pol<2; pol++){
				if(!WFRMS[pol] && !failsCWPowerCut[pol]){
					if(!isNewBox && !box300){
						if(!isSurf[0] && !isSurf[1] && !isSurfEvent_top[pol]){
							if(isVertexable[pol] && !isSurfEvent_vertex[pol]){ // add vertexability condition
								if(!failsRcut[pol]){
			...
			if(pass_this_pol[0] || pass_this_pol[1]){
				...
				pass_sq_vsE->Fill(logE,weight*weight);
			...
			thrown_sq_vsE->Fill(logE,weight*weight);
	...
	for(int pol=0; pol<2; pol++){
		for(int bin=0; bin<=all_events[pol]->GetNbinsX(); bin++){
			...
			double thrown_sq = thrown_sq_vsSNR[pol]->GetBinContent(bin);
			double pass_sq = pass_sq_vsSNR[pol]->GetBinContent(bin);
			if(thrown>0.){
				...
				double frac_err = sqrt(pow(sqrt(pass_sq)/pass_soft_short_cal_wfrms_box_surf_rcut_this,2.) + pow(sqrt(thrown_sq)/thrown,2.));
				double this_err = (pass_soft_short_cal_wfrms_box_surf_rcut_this/thrown) * frac_err;
				cout<<"The frac erorr for pol "<<pol<<" in bin "<<bin<<" is "<<frac_err<<" so the total error is "<<this_err<<endl;
				efferr_custom_vsSNR[pol]->SetBinContent(bin,this_err);				
			}
			else{
	...
	for(int bin=0; bin<=all_events_vsE->GetNbinsX(); bin++){
		...
		double thrown_sq = thrown_sq_vsE->GetBinContent(bin);
		double pass_sq = pass_sq_vsE->GetBinContent(bin);
		if(thrown>0.){
			...
			// cout<<"The erorr in bin "<<bin<<" is "<<sqrt(pow(passerr_custom_this/pass_custom_this,2.))<<endl;
			// double this_err = (pass_custom_this/thrown) * sqrt(pow(passerr_custom_this/pass_custom_this,2.));
			// cout<<"The erorr in bin "<<bin<<" is "<<sqrt(pow(sqrt(pass_custom_this)/pass_custom_this,2.) + pow(sqrt(thrown)/thrown,2.))<<endl;
			// double this_err = (pass_custom_this/thrown) * sqrt(pow(sqrt(pass_custom_this)/pass_custom_this,2.) + pow(sqrt(thrown)/thrown,2.));
			double frac_err = sqrt(pow(sqrt(pass_sq)/pass_custom_this,2.) + pow(sqrt(thrown_sq)/thrown,2.));
			double this_err = (pass_custom_this/thrown) * frac_err;
			// cout<<"The frac erorr in bin "<<bin<<" is "<<frac_err<<" so the total error is "<<this_err<<endl;
			efferr_custom_vsE->SetBinContent(bin,this_err);
		}
		else{
			...
			efferr_custom_vsE->SetBinContent(bin,0.);
		}
	}
	...
	sprintf(pretty_title,"/users/PAS0654/osu0673/A23_analysis_new2/results/paper_work/a%d_c%d_eff_vs_snr.pdf",station,config);
	cEff->SaveAs(pretty_title);


	for(int snrBin=5; snrBin<=eff_soft_short_cal_wfrms_box_surf_rcut[0]->GetNbinsX(); snrBin++){
		...
		double eff_err = efferr_custom_vsSNR[0]->GetBinContent(snrBin);
		// printf("VPol Efficiency for SNR bin %d with center %.2f is %.3f with error %.3f \n",snrBin,binCenter,eff,eff_err);
		// printf("%.4f, %.4f \n",eff,eff_err);
		printf("%.4f\n",eff);
	}
	printf("\n\n");
	...
	TCanvas *cEffVsE = new TCanvas("eff_vs_energy","eff_vs_energy",1.1*850,850);
		...
		// leg2->Draw();
	sprintf(pretty_title,"/users/PAS0654/osu0673/A23_analysis_new2/results/paper_work/a%d_c%d_eff_vs_energy.pdf",station,config);
	cEffVsE->SaveAs(pretty_title);

	// this one is wrong, just FYI
	...
	sprintf(pretty_title,"/users/PAS0654/osu0673/A23_analysis_new2/results/paper_work/a%d_c%d_eff_vs_energy_test.pdf",station,config);
	...
	for(int eBin=1; eBin<=eff_custom_vsE->GetNbinsX(); eBin++){
		...
		double eff_err = efferr_custom_vsE->GetBinContent(eBin);
		// printf("Test Efficiency for energy bin %d with center %.2f is %.3f with error %.3f \n",eBin,binCenter,eff,eff_err);
		printf("%.5f, %.5f\n",eff, eff_err);
	}
