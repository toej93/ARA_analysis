////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
////	save_info_vs_intercept_sourceSearch.cxx 
////	A23 source, make plots of the final cut parameter space
////	updated for use with newest cuts
////	
////
////	May 2021
////	Created by B. Clark, adapted by J. Torres for a source search
////////////////////////////////////////////////////////////////////////////////
...
int Optimize(int station, int config, int pol_select, double slope, int numBins_in, double intercepts_in[250], double (&background_out)[250], double (&signal_out)[250], double (&S_up_out)[250], double (&fitPar_out)[2], double (&fitParErr_out)[2], double &pval_out, double &cov_out, double &cor_out);
...
int main(int argc, char **argv)
{
	...
	OutputTree->Branch("S_up",&S_up,"S_up[250]/D");
	...
	for(int this_config=1; this_config<2; this_config++){
		...
		fitConverge = Optimize(station,this_config, pol_select, slope, 250, intercept, backgrounds, signal, S_up, fitPar, fitParErr, pVal, covariance, correlation);
...
int Optimize(int station, int config, int pol_select, double slope, int numBins_in, double intercepts_in[250], double (&background_out)[250], double (&signal_out)[250], double (&S_up_out)[250], double (&fitPar_out)[2], double (&fitParErr_out)[2], double &pval_out, double &cov_out, double &cor_out){
	...
	// if(station!=3){
	// 	printf(RED"You have asked for station %.2f, butt this code explicitly designed to work only for A3.\n"RESET,station);
	// 	return -1;
	// }
	...
	// sprintf(the_data,"/fs/project/PAS0654/ARA_DATA/A23/10pct_redo/ValsForCuts_v2/A%d/c%d/273*.root",station,config); //latest
	sprintf(the_data,"/fs/project/PAS0654/ARA_DATA/A23/10pctCalibBugFix/ValsForCuts/A%d/c%d/cutvals_drop_FiltSurface_CWThresh2.0_snrbins_0_1_wfrmsvals_0.0_0.0_run_*.root",station,config);//Source search
	...
	// do this inside brackets for scoping power and re-use of identical variable names when it comes time for simulation to happen
	{
		...
		dataAllTree.SetBranchAddress("bad_updated",&bad_v2);
		dataAllTree.SetBranchAddress("isRFEvent",&isRFEvent);
		dataAllTree.SetBranchAddress("isPayloadBlast2",&isPayloadBlast2);
		// dataAllTree.SetBranchAddress("box300",&box300);//contained in "isBox"
		dataAllTree.SetBranchAddress("runNum",&runNum);
		dataAllTree.SetBranchAddress("badRun",&badRun);
		dataAllTree.SetBranchAddress("neutrinoBox",&isInNeutrinoBox);
		dataAllTree.SetBranchAddress("CenA_OnSP",&isCenA_OnSP);
		dataAllTree.SetBranchAddress("isInControlSample",&isInControlSample);
		dataAllTree.SetBranchAddress("isCrossCheck",&isCrossCheck);
		...
		for(int event=0; event<numDataEvents; event++){
			...
			if(runNum!=currentRunNum){
				...
				isThisASoftDomRun = isSoftwareDominatedRun(toolsPath, station, runNum);
				if(isThisABadRun){
					printf(RED"*"RESET);
			...
			if(!isInControlSample || isInNeutrinoBox || isCenA_OnSP){//Source search related stuff
					continue;
			}
	...
	*/
	...
	for(int bin=0; bin<numBins_in; bin++){
		...
		// S_up_out[bin] = GetS_up_TMath(background_out[bin],achieved_alpha, 0.9); // compute S_up for this background
		S_up_out[bin] = GetS_up(background_out[bin],achieved_alpha, 0.9);
	}
