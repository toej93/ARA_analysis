    ...
    OutputTree[0] = new TTree("OutputTreeV","OutputTreeV");
		...
		for(int file_num=6; file_num<argc; file_num++){
  		...
  		bool badRun;
			...
			trees[2]->SetBranchAddress("bad_updated",&bad_v2);
			...
			trees[2]->SetBranchAddress("runNum",&runNumIn);
			...
			bool isThisABadRun = false;
			bool isThisASoftDomRun= false;
			...
			for(int event=0; event<numEntries; event++){
				...
				if(!isRFEvent || isThisASoftDomRun || isSpikey || isCliff || OutofBandIssue || bad_v2 || isPayloadBlast){
					continue;
				}
        ...
        simOutTree.GetEvent(event);
          ...
          pass0=false;
					...
					if(!WFRMS[pol] && !failsCWPowerCut){
						if(!isNewBox){
							if(!isSurf[pol] &&/* !isSurf[1] &&*/ !isSurfEvent_top[pol] && isInNeutrinoBox){//Only check surface cut with whatever polarization we're running.
