...
int Optimize(int station, int config, int pol_select, double slope, int numBins_in, double intercepts_in[200], double (&background_out)[200], double (&signal_out)[200], double (&S_up_out)[200], double (&fitPar_out)[2], double (&fitParErr_out)[2], double &pval_out, double &cov_out, double &cor_out);
...
int main(int argc, char **argv)
{
	...
	OutputTree->Branch("S_up",&S_up,"S_up[200]/D");
	...
	// for(int this_config=3; this_config<4; this_config++){
		...
		fitConverge = Optimize(station,this_config, pol_select, slope, 200, intercept, backgrounds, signal, S_up, fitPar, fitParErr, pVal, covariance, correlation);
		OutputTree->Fill();
	}
	OutputFile->Write();
	OutputFile->Close();
