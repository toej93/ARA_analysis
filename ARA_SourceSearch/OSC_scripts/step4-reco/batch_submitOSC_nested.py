...
for year in range(2016,2017):
    ...
    with open(file, mode='r', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for job in reader:
            if(count_core==0):
                f = open("/users/PAS0654/osu8354/ARA_cvmfs/source/AraRoot/analysis/ARA_analysis/ARA_SourceSearch/OSC_scripts/step4-reco/tmpScipts/A%i_%i_rad%i_tmpSubmit_%i.sh"%(station, year,radius, count), "w+")
                f.write("#!/bin/bash\n\n")
