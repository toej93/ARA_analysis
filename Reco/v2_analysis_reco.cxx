...
int main(int argc, char **argv)
{
	...
	*/
	...
	if(numEntries>6000){
		cout << "More than 6000 events" << endl;
		cout << argv[7] << endl;
		ofstream myfile ("interf_redo.txt");
  	if (myfile.is_open()){
    	myfile << argv[7] << "\n";
    	myfile.close();
  	}
	...
	OutputTree->Branch("peakTheta_single", &peakTheta_single, "peakTheta_single[2]/D");
	OutputTree->Branch("peakPhi_single", &peakPhi_single, "peakPhi_single[2]/D");
	...
	for(Long64_t event=0;event<20;event++) {
		if(event%starEvery==0) {
			std::cout << "*";
		}
		if (event>1 && event%2000 == 1){
			int bytesWritten = OutputSettingsTree->AutoSave("FlushBaskets");
			int bytesWritten2 = OutputTree->AutoSave("FlushBaskets");
		 	OutputTree->FlushBaskets();
			OutputSettingsTree->FlushBaskets();
			cout << "\nBytes written to OutputSettingsTree: " << bytesWritten << endl;
			cout << "\nBytes written to OutputTree: " << bytesWritten2 << endl;
			cout << "Flushing baskets and autosaving" << endl;
		}
      ...
      peakPhi_single[1]=-1000.;
			...
			printf("Phi peak is: %f, Theta peak is: %f\n", peakPhi_single[0], peakTheta_single[0]);
			...
			if(print_maps){
				...
				sprintf(save_temp_title,"/users/PCON0003/cond0068/ARA/AraRoot/analysis/plots/interf_maps/%d.%d.%d_Run%d_Ev%d_Maps_FromRecoCode.png",year_now,month_now,day_now,runNum,event);
