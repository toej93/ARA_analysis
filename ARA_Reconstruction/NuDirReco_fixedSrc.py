"""
#####NuDirReco_fixedSrc.py#####

Directional reconstruction of neutrino events with Python
Author: Jorge Torres
Date: Jan 7, 2020.
"""
...
for filename in os.listdir("/fs/scratch/PAS0654/jorge/sim_results/1E18FixedNnu"):#Loop over desired directory
        if filename.startswith("AraOut.fixed_A2_1E19.txt.run%s.root"%(sys.argv[1])): #extension, .root in this case
            file_list.append(os.path.join("/fs/scratch/PAS0654/jorge/sim_results/1E18FixedNnu", str(filename))) #add file name to the list
...
for i in range(0,totalEvents):#loop over events
    ...
    theta_reco.append(np.radians(180-vertex[1]))
    phi_reco.append(np.radians(vertex[2]))
    ...
    # print(phi_reco)
    # print(vertex[1])
original_df = pd.DataFrame({"EvNum":np.array(evt_num),"theta_reco": np.array(theta_reco), "phi_reco":np.array(phi_reco),"theta_antenna":np.array(theta_antenna),"phi_antenna":np.array(phi_antenna),"PolTrue":PolVecTrue_array,"PolReco":PolVecReco_array,"rmsV":np.array(rmsV),"rmsH":np.array(rmsH),"maxV":np.array(maxV_array),"maxH":np.array(maxH_array),"powerV":np.array(powerVArr),"powerH":np.array(powerHArr),"energyArr":np.array(energyArr),"weight":np.array(weight_arr),"view_ang":np.array(view_ang),"peak_V":np.array(Peak_V),"peak_H":np.array(Peak_H),"dirProp":dirProp,"nnu":nnu,"launch_ang":launch_ang})
original_df.to_pickle("./1E18FixedNnu/1E19FixedNnu_%s.pkl"%(int(sys.argv[1])))
